...
function App() {
  ...
  useEffect(() => {
    const getPageInfo = async () => {
      ...
      const groupByRiskLevel = cspDirectives.reduce(
        (acc, directive: any) => {
      ...
      const riskLevelAmount = joinedDirectives.reduce(
        (acc, directive) => {
          const riskLevel =
            directive.riskLevel.toLowerCase() as keyof typeof acc;
          acc[riskLevel] += 1;
          return acc;
        },
  ...
  return appInfo ? (
    <div className="flex flex-col items-start justify-start p-6 gap-4">
      <div className="flex flex-col gap-2 w-full">
        ...
        <div className="flex items-start gap-8 justify-content-between align-content-between w-full border dark:border-slate-700/60 border-slate-200 rounded-md px-4 py-2">
                      ...
                      <li
                        ...
                        className={`flex flex-col gap-2 mb-4 ${
                          index < directives.length - 1
                            ? "border-b dark:border-slate-700/60 border-slate-200"
                            : ""
                        }`}
                      >
    ...
    <div className="flex flex-col items-start justify-start p-6 gap-4 w-full">
      <div className="flex items-center justify-start gap-2">
